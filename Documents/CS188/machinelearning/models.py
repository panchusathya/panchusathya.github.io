...
class RegressionModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        #hidden layer between 100 and 500
        # batch between 1 and 128
        # lerning rate between 0.0001 and 0.01
    ...
    def forward(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        hidden = relu(self.b1(x))
        return self.b2(hidden)
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        h1 = 169
        h2 = 269
        self.b1 = Linear(input_size,h1)
        self.b2 = Linear(h1,h1)
        self.b3 = Linear (h1, output_size)
    ...
    def forward(self, x):
        ...
        """ YOUR CODE HERE """
        hidden = relu(self.b1(x))
        hidden_2 = relu(self.b2(hidden))
        return self.b3(hidden_2)
...
class LanguageIDModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        # Initialize your model parameters here
        h1 = 269 # hidden layer 1
        #h2 = 269
        self.layer1 = Linear(self.num_chars,h1)
        self.layer2 = Linear(h1, h1)
        self.layer3 = Linear(h1, 5)
    ...
    def forward(self, xs):
        ...
        h = zeros(batch_size, 269)
        for word in xs:
            one = self.layer1(word)
            two = self.layer2(h)
            h = relu(one + two)
...
def Convolve(input: tensor, weight: tensor):
    ...
    "*** YOUR CODE HERE ***"
    #Output_Tensor = zeros((weight_dimensions))
    #Output_Tensor = tensordot(input, weight, dims = (input_tensor_dimensions, weight_dimensions))
    input_height, input_width = input_tensor_dimensions
    weight_height, weight_width = weight_dimensions
    Output_Tensor = zeros((input_height-weight_height+1, input_width-weight_width+1))
    for x in range(input_height-weight_height+1):
        for y in range(input_width-weight_width+1):
            new_weight = input[x:x+weight_height, y:y+weight_width]
            Output_Tensor[x,y] = tensordot(weight,new_weight, dims=([0,1], [0,1]))
    "*** End Code ***"
...
class Attention(Module):
    ...
    def forward(self, input):
        ...
        mask = matmul(Q, K.transpose(1, 2))/math.sqrt(C)
        M = mask.masked_fill(self.mask[:,:,:T,:T] == 0, float('-inf'))[0]
        #M = movedim(M, C, T)
        soft = softmax(M, dim=-1)
